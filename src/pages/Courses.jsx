...
export function Courses() {
  useDocumentTitle('Курсове')
  const { items } = useLocalCollection('courses', courses)
  const [query, setQuery] = useState('')
  const [selectedCourse, setSelectedCourse] = useState(null)
  const published = useMemo(() => items.filter((course) => course.is_published), [items])
  ...
  return (
    <>
      <PageHero
        eyebrow="Обучения"
        title="Сертифицирани курсове за специалисти"
        text="Програми за консултиране, психотерапевтични умения и психопатология с практическа насоченост."
      >
        <div className="max-w-xl">
          <SearchField value={query} onChange={setQuery} placeholder="Търсене по тема, заглавие или продължителност" />
        </div>
      </PageHero>
            ...
            filtered.map((course) => (
              <div key={course.id} className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
                ...
                <div className="premium-panel p-6">
                  <h2 className="text-2xl font-semibold text-[#153b34]">Тематични направления</h2>
